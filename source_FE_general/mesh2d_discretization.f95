MODULE mesh2d_discretization
  ...
  USE derived_data_module, ONLY: nodes, dofs, geometry_node_type, deep_node_copy_geometry2EM
  ...
  USE FORTRAN_generic, ONLY: add_to_list_integer, find_intersect_integer, print_general_msg,&
       print_debug_msg, whether_two_integer_arrays_identical
  ...
  PUBLIC :: p2node, p2ele, ele2node, ele2edge, ele2dof, edge2node, eg2eg, eg2ele, cellsigma

  ! basic info for unstructured meshes
  INTEGER,ALLOCATABLE :: ele2node(:,:), neighele(:,:)  
  INTEGER,ALLOCATABLE :: edge2node(:,:), ele2edge(:,:), ele2dof(:,:)

  REAL(DPR),ALLOCATABLE :: cellsigma(:)

  ! quantity (e.g., a node, an edge, or element) to all share-the-quantity cells' indexes
  TYPE pt2ele_type
...
CONTAINS
  ...
  SUBROUTINE read_node_file(filepath, filename, nodeobj)
    ! here, "node" and "point" refer to the same thing: vertex in a mesh.
    IMPLICIT NONE

    INTEGER :: fid, npoint, i, inull
    CHARACTER(LEN=*),INTENT(IN) :: filepath, filename
    TYPE(geometry_node_type),INTENT(INOUT),ALLOCATABLE :: nodeobj(:)
  ...
  SUBROUTINE check_edge_boundary(nodeobj, edgeobj)
    ...
    IMPLICIT NONE
    INTEGER :: i, nedge
    TYPE(geometry_node_type),INTENT(IN) :: nodeobj(:)
    INTEGER,INTENT(INOUT),ALLOCATABLE :: edgeobj(:,:)
  ...
  SUBROUTINE calculate_pt2pt_information(np, neiobj, eleobj, pt2ele_obj, pt2pt_obj)
    ! find the connectivity among nodes (to elements, to edges) based on the available mesh information.
    IMPLICIT NONE
    INTEGER :: i
    ...
    CALL print_general_msg('Searching calculation for local pt2ele and pt2pt connectivity information')
    CALL print_debug_msg('total # of vertices',np)
  ...
  SUBROUTINE calculate_element_2_edge_information(ele2node_obj, edge2node_obj, ele2edge_obj)
    ...
    INTEGER :: errflag, k, m, i, nele, nedge, pos

    ! boundary mark value (1 or 2) for each edge when that edge is already found to belong to a cell;
    ! this is to speed up calculation;
    ! value == 1: if a boundary edge has been matched to a cell since this kind of edges can only belong to
    !   one cell;
    ! value == 2: if an interior edge has been matched to all two cells.(If all 2 cells sharing this edge
    !   have been found.)
    INTEGER,ALLOCATABLE :: mark_edge(:)  
    nele = SIZE(ele2node_obj, 1)
    nedge = SIZE(edge2node_obj, 1)
    ALLOCATE( mark_edge(nedge) ); mark_edge = 0
    IF( ALLOCATED(ele2edge_obj) ) DEALLOCATE(ele2edge_obj); ALLOCATE( ele2edge_obj(nele, 3) )

    CALL print_general_msg('Searching calculation for element-to-edge information...')
    ! brute-force search way
    NS: DO k = 1, nele
       CALL percentage_display(k, nele)
       m = 0  ! # of found edges
       ND: DO i = 1, nedge
          IF( mark_edge(i) >= 2 )   CYCLE ND
          IF( mark_edge(i) == 1 .AND.  edge2node_obj(i, 3) /= 0  ) CYCLE ND
          ...
          IF( errflag ==0 .AND. (SIZE(intersect) == 2) ) THEN
             ! a shared edge is found
             m = m + 1
             ! place this edge in the 'right' position (hierarchical order, see later descriptions)
             pos = 0
             IF( whether_two_integer_arrays_identical(edge2node_obj(i,1:2), ele2node_obj(k, 1:2)) ) pos = 1
             IF( whether_two_integer_arrays_identical(edge2node_obj(i,1:2), ele2node_obj(k, 2:3)) ) pos = 2
             IF( whether_two_integer_arrays_identical(edge2node_obj(i,1:2), [ele2node_obj(k, 3),ele2node_obj(k, 1)]) ) pos = 3
             ele2edge_obj(k, pos) = i
             mark_edge(i) = mark_edge(i) + 1
          END IF
          IF( m == 3) CYCLE NS    ! no need to iterate over the rest of edges; save time
       END DO ND
    END DO NS

    ! re-ordering the 3 edges of each cell such that they follow the hierarchical order
    ! i.e., e-1 corresponds to node-1 and node-2; e-2 corresponds to node-2 and node-3;
    !  e-3 corresponds to node-3 and node-1
    ! Above, e-1, e-2, e-3 are 3 edges
    RETURN
  ...
  SUBROUTINE calculate_local_edge_connectivity(edgeobj, ele2eg_obj, pt2ele_obj, eg2ele_obj, eg2eg_obj)
    ...
    CALL print_general_msg('searching calculation local edge-2-edge connectivity information ...')
  ...
  SUBROUTINE setup_dofs(ndof)
    ! coordinates, identification of boundary dofs: set up dofs, ele2dof
    ! should be called after reading reading mesh info
    IMPLICIT NONE
    INTEGER,INTENT(INOUT) :: ndof
    INTEGER :: k, id, nvert, nedge, nele, nv1, nv2

    IF(ALLOCATED(dofs))  DEALLOCATE( dofs )
    IF(ALLOCATED(ele2dof))  DEALLOCATE( ele2dof )

    nvert = SIZE(nodes)
    nele = SIZE(ele2node, 1)
    deg: IF(modelling%FEdegree == 1) THEN
       ndof = SIZE(nodes)
       ALLOCATE( dofs(ndof) )
       DO k = 1, ndof
          CALL deep_node_copy_geometry2EM(nodes(k), dofs(k))
       END DO

       ALLOCATE( ele2dof(nele, 3) )
       ele2dof = ele2node(:, 1:3)
    ELSEIF( modelling%FEdegree == 2 ) THEN
       ...
       DO k = 1, ndof
          IF( k <= nvert ) THEN
             CALL deep_node_copy_geometry2EM(nodes(k), dofs(k))
          ELSE
             ! reg infor for dofs of this type undefined !
             id = k - nvert
             nv1 = edge2node(id, 1)
             nv2 = edge2node(id, 2)
             dofs(k)%x = ( nodes(nv1)%x + nodes(nv2)%x ) * 0.5
             dofs(k)%z = ( nodes(nv1)%z + nodes(nv2)%z ) * 0.5
             IF( edge2node(id, 3) == 1 ) THEN
                dofs(k)%bud = 1
             ELSE
                dofs(k)%bud = 0
             ENDIF
       ...
       ALLOCATE( ele2dof(nele, 6) )
       DO k = 1, nele
          ele2dof(k, 1:3) = ele2node(k, 1:3)
          ele2dof(k, 4:6) = ele2edge(k, 1:3) + nvert
       end DO          
    END IF deg

    ! always set y = 0 for 2D dofs
    dofs%y = 0.0

    RETURN
  end SUBROUTINE setup_dofs
