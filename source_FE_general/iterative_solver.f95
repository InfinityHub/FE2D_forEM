Module Iterative_solvers
  ...
  USE FORTRAN_generic, ONLY: add_to_list_real, print_general_msg, print_debug_msg
  
  IMPLICIT NONE

  ! calling interface (visible to outside programs)
  PUBLIC:: iter_solver_real,  iter_solver_complex
...
CONTAINS
  ...
  SUBROUTINE iter_solver_complex(norder, nz, irn, jcn, array, rhs, accelorator, sol)
    ...
    REAL(DPR):: re_array(4*nz), re_rhs(2*norder), re_sol(2*norder)
    INTEGER :: irNew( nz*4 ), jcNew(nz*4)


    CALL print_debug_msg('Entering iterative solver: ', TRIM(ADJUSTL(accelorator)) )
    CALL print_debug_msg('nnz = ', nz)
    ...
    CALL iter_solver_real( 2*norder, nz*4, irNew, jcNew, re_array, re_rhs, accelorator, re_sol)

    sol = 0.0
    
    sol = CMPLX( re_sol( 1: norder ), 0.0, KIND=DPR)

    sol = sol + CMPLX( 0.0, re_sol( norder +1 : norder *2 ), KIND=DPR)
  ...
  SUBROUTINE iter_solver_real(norder, nnzero, irn, jcn, array, rhs, iterSolverType, sol)
    ...
    REAL(DPR),INTENT(IN)   :: rhs(norder)
    REAL(CDPR),INTENT(INOUT)   :: sol(norder)   ! the solution array
    ...
    ilu%lfil = 3;   CALL print_debug_msg('ILU precond level of filling is set to be: ', ilu%lfil )
    ...
    solve%maxiter = 12000 ;   CALL print_debug_msg('>>> Max iterates is set to be: ', solve%maxiter )

    solve%nKrylov = 1000;  CALL print_debug_msg('>>> Krylove subspace size is set to be: ', solve%nKrylov)
    ...
    RETURN
  END SUBROUTINE iter_solver_real
